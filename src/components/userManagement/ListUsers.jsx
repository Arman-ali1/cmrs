...
function ListUsers({ user, onUpdate }) {
    ...
    fetch("https://crms-chat-backend.onrender.com/api/chats")
      ...
      .then((data) => {
        ...
        const conversation = data.filter(
          (msg) =>
            (msg.userId === adminId && msg.targetUserId === user.id) ||
  ...
  const handleVerification = () => {
    ...
    axios
      ...
      .then((response) => {
        console.log("User verification updated successfully:", response.data);
    ...
    <div className="col-md-4 mb-4">
      ...
      >
        ...
        <div className="card-body">
          ...
          <p className="card-text">{user.email || "email@example.com"}</p>
          <a
          ...
          <button onClick={handleEdit} className="btn btn-warning btn-sm ms-2">
            Update
          </button>
          <button
  ...
  className={`btn btn-sm ms-2 position-relative ${
    chatButtonLabel === "Pending" ? "btn-warning" : "btn-warning"
  }`}
  onClick={handleChat}
>
  {/* Button Text */}
  {chatButtonLabel === "Pending" ? "Pending" : chatButtonLabel}
  ...
  {chatButtonLabel === "Pending" && (
    ...
    >
      1+
          ...
          <button
  className={`btn btn-sm ms-2 ${isVerified ? "btn-primary" : "btn-warning"}`}
...
export default function UsersList() {
  ...
  return (
    <div className="container">
      <div className="row">
        {users.map((user) => (
          <ListUsers key={user.id} user={user} onUpdate={setSelectedUserId} />
        ))}
      </div>
    </div>
  );
